...
M.toggle_quicklist = function() if fn.empty(fn.filter(fn.getwininfo(), 'v:val.quickfix')) == 1 then vim.cmd('copen') else vim.cmd('cclose') end end
...
M.jobstart = function(cmd, on_finish)
  ...
  local function on_event(_, data, event)
    if event == "stdout" then
      ...
      for i = 1, #data do table.insert(lines, data[i]) end
  ...
  vim.fn.jobstart(cmd, { on_stderr = on_event, on_stdout = on_event, on_exit = on_event, stdout_buffered = true, stderr_buffered = true, })
end
