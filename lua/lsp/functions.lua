...
function M.enable_format_on_save()
	...
	vim.api.nvim_create_autocmd("BufWritePre", { callback = function() vim.lsp.buf.format() end, group = group, })
...
function M.toggle_format_on_save()
	if vim.fn.exists("#format_on_save#BufWritePre") == 0 then M.enable_format_on_save() else M.disable_format_on_save() end
end
...
function M.custom_hover_handler(_, result)
	local handler = function(_, result)
		if result then
			...
			local bufnr = vim.lsp.util.open_floating_preview(lines, "markdown", { border = xotovim.ui.float.border or "single" })
